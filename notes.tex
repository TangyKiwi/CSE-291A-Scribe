...
This example demonstrates a general principle: "If every part of an operation keep the same AI, the overall AI remains the same."
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.70\textwidth]{images/matmul.png}
    \caption*{Improvements in MatMul runtime over time}
\end{figure}
...
Consequently, we have the following memory hierarchy, from fastest to slowest:
...
For the write cost, $c$ is still $n^2$, since we still need to store every element regardless of how we tile. As a result, only the read time is improved. Interestingly, this derived cost equation shows that load cost is not related to v3.
...
\section*{Scribe Responsibilities}
\begin{itemize}
    \item{Samar Karanch}: Section 2, Images
    \item{Kevin Lin}: Outline + Code Blocks, Section 3 
    \item{Owen Park}: Section 2, 3
    \item{Shaurya Raswan}: Outline, Proofreading, Images
    \item{Julie Wu}: Proofreading
    \item{Jaewoong Yun}: Section 1, 4
    \item{Ian Jayachandran}: Proofreading and minor additions
\end{itemize}
All work can be tracked through the GitHub repository: \href{https://github.com/TangyKiwi/CSE-291A-Scribe}{https://github.com/TangyKiwi/CSE-291A-Scribe}
